...
| `WT_SECURE_IMAGE_HEADER_SIZE` | Bytes reserved before linked code; default `0`. | Set by the authenticated wolfBoot image layout when a header is present. |
...
The authenticated target runners override standalone addresses for the
wolfBoot partition layout. Keep bootloader, Secure image, manifest, guest
linker files, measurement records, and flash commands consistent.
...
See [[Building]] for command examples and [[Security Model]] for the runtime
