...
static int veneer_precheck(uint32_t *out_vm)
{
    uint32_t vm;
...
static int do_open(vnet_info_t *ns_info)
{
    ...
    rc = vnet_switch_open(&g_switch, vm, &local);
    if (rc != WT_VNET_OK) return rc;
...
static int do_set_mac(const uint8_t *ns_mac6, uint32_t flags)
{
    ...
    return vnet_switch_assign_mac(&g_switch, vm, &mac);
}
...
static int do_tx(const void *ns_frame, uint16_t len, uint32_t flags)
{
    ...
    return vnet_switch_tx(&g_switch, vm, scratch, len, now_tick());
}
...
static int do_rx_poll(vnet_rx_meta_t *ns_meta)
{
    ...
    rc = vnet_switch_poll_rx(&g_switch, vm, &local);
    if (rc != WT_VNET_OK) return rc;
                      ...
                      void *ns_dst, uint16_t dst_len)
{
    ...
    n = vnet_switch_read_rx(&g_switch, vm, slot, gen, scratch, dst_len);
    if (n < 0) return n;
...
static int do_rx_release(uint16_t slot, uint16_t gen)
{
    ...
    int rc = veneer_precheck(&vm);
    if (rc != WT_VNET_OK) return rc;
    return vnet_switch_release_rx(&g_switch, vm, slot, gen);
}
...
static int do_irq_ack(void)
{
    ...
    int rc = veneer_precheck(&vm);
    if (rc != WT_VNET_OK) return rc;
    return vnet_switch_irq_ack(&g_switch, vm);
}
