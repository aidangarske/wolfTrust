...
This guide builds the STM32H563 Secure image, the Zephyr and FreeRTOS reference
guests, and the host tests. See [[Building]] for all supported targets, and read
[[STM32H5 Guide]] before flashing a board.
...
Guest setup downloads pinned Zephyr and FreeRTOS sources, so it requires
...
isolation; see [[Testing]] for the validation split.
...
bytes first as described in [[STM32H5 Guide]]. The current
...
- Read [[Architecture]] for the request path and isolation model.
- Use [[API Reference]] when integrating a PSA client.
- Use [[Macros]] before changing the target profile.
- Use [[Porting]] when adding another Armv8-M device.
