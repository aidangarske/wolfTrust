...
independent TF-M isolation certification. [[Security Model]] describes the
actual boundary.
...
3. Initial Attestation currently has only the Zephyr-specific adapter at
   ...
   The vendored `lib/wolfPSA/src/psa_attestation.c` is a not-supported stub and
   must not be linked instead of or alongside that adapter.
