...
| Initial Attestation | `lib/wolfPSA/wolfpsa/psa/initial_attestation.h` | 1.0 API operations; see [[TF-M Compatibility]] for API and token-profile deviations |
...
reference guests. Key types, algorithms, multipart operations, and optional
extensions compile only when enabled by the active `user_settings.h`.
An API declaration is not evidence that every algorithm is enabled.
...
The vendored `lib/wolfPSA/src/psa_attestation.c` is a not-supported stub; do not
link it instead of or alongside the Zephyr adapter because both define the same
public PSA symbols.
...
For service availability and deviations, see [[Services]] and
[[TF-M Compatibility]].
