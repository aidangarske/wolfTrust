...
- **Protected service keys.** The Initial Attestation Key is generated in the wolfHSM server keystore as non-exportable and used there for signing, and the device-unique storage-sealing key is also non-exportable; imported private material, by contrast, necessarily originates in caller memory before the server accepts it.
