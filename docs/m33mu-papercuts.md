...
- STM32H563 RNG peripheral fidelity used to be missing in `m33mu` because the
  secure RNG alias checked the non-secure RCC gate state instead of `RCC_S`.
  With that fixed in `m33mu`, the wolfHAL STM32H5 TRNG path now runs under the
  emulator without `WT_INSECURE_TEST_RNG=1`. Keep a reduced RNG regression test
  in mind if the model grows beyond the current data-ready behavior.
