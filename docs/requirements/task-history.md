...
- **ACTIVE PROGRAM - wolfTrust Secure Manager replacement, the H5 port mega plan
  ...
  - MP5 [ ] Drop-in proof ON SILICON: the unmodified Arm `val` NSPE FF-M IPC
    ...
    - MP5-A [x] Wire `confboot` into `run_h5_hardware.sh` (2026-08-19):
      `WT_CONFORMANCE=1` secure + `WT_RUN_CONFORMANCE=1` guest, report-terminated
      UART capture across the reboot loop, no emulator BKPT; ACS-report-block
      gate (boot banners splice mid-word across 92 reboots); explicit
      `pyocd reset` after flash (CubeProgrammer `-hardRst` unreliable);
      `confboot` added to the `run_h5_suite.sh` default set.
    - MP5-B [~] DROP-IN PROVEN ON SILICON, gate hardening OPEN (2026-08-19):
      the unmodified Arm suite reached **89 / 85 passed / 0 failed / 4 skipped**
      on the NUCLEO-H563ZI three times (manual + two back-to-back), 92 real
      SYSRESETREQ reboot cycles. Three silicon defects found+fixed en route
    ...
    - MP5-C [~] ST Secure Manager side-by-side DESCOPED (2026-08-19, owner
      decision): the drop-in proof stands on the unmodified Arm conformance suite
      on silicon; the H573I-DK baseline is dropped (H563 can't run SM, board not
      owned).
