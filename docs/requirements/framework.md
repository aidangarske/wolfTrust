...
| ID | Behavior | Failure | Source | Tests | Commit |
| --- | --- | --- | --- | --- | --- |
| WT-FFM-0040 | Framework-version discovery derives from the loaded manifest and is clamped to the compiled public contract (PSA_FRAMEWORK_VERSION), so every caller path of one build — Non-secure discovery, Secure Partition query, and public headers — reports the same version; a 1.1 report requires a 1.1 public contract with its isolation level and memory-mapped IOVEC availability. | Feature discovery never advertises an unenforced capability, and no build reports different framework versions to different callers. | SRC-FFM-EXT 2.1 | Feature-discovery tests (host `ffm` derive-and-clamp cases) | 5ecc631 |
...
| WT-FWU-0002 | A staged candidate image is written to the wolfBoot update partition and armed for swap on next boot, and the swapped image runs only after passing authenticated launch and anti-rollback. The public psa_fwu_* 1.0 surface (query, start, write, finish, cancel, clean, install, reject, request_reboot) drives this flow; installation commits at the reboot, so the optional TRIAL flow is not offered. | A candidate that fails the staged write, arming, or a lifecycle transition leaves the running image unchanged and reports the failure through query. | SRC-PSA-FWU; WT-FFM-0049, WT-FFM-0050 | Staged-swap and gated-launch tests; public-API round trip (`tests/host/psa_ffm_client`); `fwustage` reject/clean tail on target | |
...
| ID | Behavior | Failure | Source | Tests | Commit |
| --- | --- | --- | --- | --- | --- |
| WT-FFM-0060 | SERVICE_ATTEST executes as a scheduled Secure Partition with its own managed execution thread and stack, servicing every Initial Attestation request through the SPM transport rather than dispatching inline on the SPM boot context. | Attestation dispatch that runs on the SPM boot context instead of a scheduled partition thread, or that cannot serve a token request over the scheduled path, fails the gate. | SRC-FFM 3.2.3; WT-FFM-0013, WT-FFM-0014 | Host SERVICE_ATTEST transport dispatch suite (`tests/host/attestation_service`); M33MU and H563 silicon `devattest` | a858a1c |
| WT-FFM-0061 | SERVICE_FWU executes as an unprivileged scheduled Secure Partition confined to its manifest protection domain; its privileged staging of the wolfBoot update partition flash (erase, program, arm, disarm) runs only through a narrow SPM gate operation that the dispatcher pins to the FWU partition identity. | A staging-backend request issued by any partition other than SERVICE_FWU is refused at the gate with no flash effect, and an unprivileged FWU access outside its domain faults the partition. | SRC-FFM 3.1.3 to 3.1.6, 3.2.3; WT-FFM-0011, WT-FFM-0013; WT-SYS-0003 | Confined FWU staging target scenarios (`fwustage`, `bootupdate` on M33MU and H563 silicon) and the cross-partition gate-pin proof (`crossdomain`) | 60f381f |
| WT-FFM-0062 | The keystore partitions (attest, relay, vault) execute as unprivileged scheduled Secure Partitions whose manifest domains grant exactly their own stacks plus the declared shared keystore band holding the wolfHSM server, NVM, and lock state; their privileged flash, entropy, and NVM-lock needs run only through SPM gate operations the dispatcher pins to the keystore partition identities. | A keystore gate request from a non-keystore partition is refused with no effect, a non-keystore partition's access to the keystore band faults the initiating partition, and an unprivileged keystore access outside its domain faults the partition. | SRC-FFM 3.1.3 to 3.1.6, 3.2.3; WT-FFM-0011, WT-FFM-0013; WT-SYS-0003, WT-SYS-0014 | Keystore confinement target scenarios (`positive`, `devcrypto`, `devstorage`, `devattest`) and the band-isolation and gate-pin proofs (`crossdomain`, `keystoreneg`), on M33MU and H563 silicon | 1922ce1 |
| WT-FFM-0063 | An FF-M PROGRAMMER ERROR committed by a Secure Partition through the SPM gate panics the calling partition in the production image: the partition never resumes with a recoverable status, terminates under its WT-FFM-0017 restart policy, and its pinned clients complete with PSA_ERROR_COMMUNICATION_FAILURE. psa_close treats only the null handle as a no-op; closing any other invalid handle, error statuses included, is such a PROGRAMMER ERROR. | A production build that returns a recoverable status to the erring Secure Partition, or whose psa_close silently accepts a non-null invalid handle, fails the gate. | SRC-FFM 3.5; WT-FFM-0014, WT-FFM-0017; WT-SYS-0008 | Gate must-panic classification (host `spm_gate` cases) and the target secure-caller-misuse scenario (`panicneg`), on M33MU and H563 silicon; `confboot` 85/0/4 on both targets with the narrowed psa_close | 492bb1f |
| WT-FFM-0064 | The Initial Attestation token carries the standard PSA profile-2 claim set at the registered CBOR keys: EAT nonce (10), UEID (256), profile (265, carrying the final RFC 9783 identifier `tag:psacertified.org,2023:psa#tfm`), and boot seed (268), plus the PSA implementation id (2396), caller client id (2394), security lifecycle (2395), and software components (2399); the certification-reference (2398) and verification-service (2400) claims are emitted only when a deployment configures real values. The boot seed hashes the boot measurement with the device instance id, so it is stable across one measured boot and reproducible for the golden vector. | A token missing a standard claim, carrying a wrong-typed claim, advertising an unconfigured optional claim, or exceeding the published maximum token size fails verification. | SRC-PSA-ATTEST-1; WT-FFM-0049 | Golden claim-set vector (`tests/host/attestation_golden`), guest0 on-target claim verification and the upstream attestation conformance suite (`positive`, `devattest`), on M33MU and H563 silicon | f045c74 |
| WT-FFM-0065 | SERVICE_VNET executes as an unprivileged scheduled Secure Partition confined to its manifest protection domain: the shared image code, its declared stack, and its declared vnet data band holding the switch state and relay staging; its time source is the scheduler tick the SVC dispatcher stamps on every gate return, and no scheduler path exists that grants a partition the whole Secure address space. | A vnet access outside its domain faults only the vnet partition under its restart policy, an execute from the non-executable vnet data band faults it likewise, and the mediated guest exchange still completes after the quarantined partition restarts. | SRC-FFM 3.1.3 to 3.1.6; WT-FFM-0011, WT-FFM-0056, WT-FFM-0058; WT-SYS-0003, WT-SYS-0015 | Confined VNET target scenarios (`vnet`, `vnetneg` cross-domain read and XN-execute negatives with restart recovery) on M33MU and H563 silicon | ebea442 |
